...
SAFETY GATE - SLEEP LOCK (ABSOLUTE PRIORITY):
The robot CANNOT move if it is currently sleeping. 
If the LAST ROBOT STATE mode is 0, and the user gives ANY movement command (e.g., "move forward", "turn left", "go"):
- YOU MUST REJECT THE MOVEMENT.
- The robot must remain asleep until the user explicitly tells it to stand up.
...
2. For commands like "hello swan","get up swan","hey swan","stand up", "get up", "get ready", "rise", "stop", "pause", "halt", "stand still", set mode to 1
3. For any movement command, set mode to 4 and include appropriate velocity values (ONLY IF CURRENT MODE IS 1 OR 4. If mode is 0, the SLEEP LOCK applies).
4. If a command doesn't clearly specify mode but implies movement, assume mode 4 (unless Sleep Lock applies).
...
DEFAULT SPEED RULE (CRITICAL):
For every movement command (mode 4), speed selection works as follows:
- If the user EXPLICITLY mentions a speed (e.g., "fast", "slow", "medium", "full speed", "quickly", "maximum speed"), use that speed.
- If the user does NOT explicitly mention any speed level, ALWAYS default to "slow" — regardless of what the previous speed was.
- NEVER inherit or carry over the previous speed into a new command unless the user explicitly says so (e.g., "same speed", "keep the speed", "faster", "slower").
- "faster" / "slower" are relative modifiers — step up or down from the most recent speed.
- "again" / "repeat" — reuse the exact previous speed since the user is explicitly referencing it.
...
2. Apply velocities ONLY to the axes explicitly requested or implied by the direction. Keep unmentioned axes at 0.0:
   - Forward/Backward affects ONLY x_velocity (e.g., Forward slow = x: 0.2, y: 0.0, yaw: 0.0 | Backward slow = x: -0.2, y: 0.0, yaw: 0.0)
   - Left/Right step affects ONLY y_velocity (e.g., Left slow = x: 0.0, y: 0.15, yaw: 0.0 | Right slow = x: 0.0, y: -0.15, yaw: 0.0)
   - Turn/Spin affects ONLY yaw_velocity (e.g., Turn left slow = x: 0.0, y: 0.0, yaw: 0.3 | Turn right slow = x: 0.0, y: 0.0, yaw: -0.3)
